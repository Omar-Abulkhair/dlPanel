...
$(function () {
    ...
    $(document).on('click', ".todo-task-list-wrapper .todo-item", function (e) {
        ...
        var $url=$(this).closest('.todo-item').attr('data-update-route')
        var $name = $(this).find('.todo-title').html();
        ...
        $('#form-edit-todo').attr('data-task-id',$id);
        $('#form-edit-todo').attr('data-update-route',$url);
        $('#form-edit-todo').find('select[name="parent_id"]').find("option[value='"+$parent_id.replace(' ','-')+"']").attr('selected',true);

        $('#form-edit-todo .edit-todo-item-title').val($name);
    ...
    $(document).on('click', '.todo-item-delete', function (e) {
        var item = this;
        e.stopPropagation();
        let id = $(item).closest('.todo-item').attr('data-task-id');
        //let route = $(item).attr('data-route');
        let route = $(item).closest('li').attr('data-delete-route');
        $.ajax({
            //data: {id: id},
            method: "DELETE",
            url: route,
    ...
    $(document).on('click', '.todo-item input', function (event) {
        ...
        let $this=$(this);
        let $id=$(this).closest('.todo-item').attr("data-task-id");
        let $url=$(this).closest('.todo-item').attr("data-status-route");
        $.ajax({
            data:{id:$id},
            method:"GET",
            dataType: "json",
            ...
            if(e.status==true){
                toastr.success('You have completed ' + $(this).parent().nextAll('.todo-title').html(), 'Congratulation Task Completed .');
...
$('#addTaskBtn').on('click', function (e) {
    ...
    $.ajax({
        ...
        if (m.status === true) {
            ...
            // HTML Output
            if (todoTitle != "") {
                $(".todo-task-list-wrapper").append('<li class="todo-item" style="animation-delay: 0s;"  data-toggle="modal" data-target="#editTaskModal"  data-parent-id="'+m.data_parent_id+'" data-delete-route="'+m.data_delete_route+'" data-delete-route="'+m.data_delete_route+'" data-status-route="'+m.data_status_route+'">' +
...
$('#updateTaskBtn').on('click', function (e) {
    e.preventDefault();
    let form = $('#form-edit-todo');
    var url = $(this).closest('#form-edit-todo').attr('data-update-route');
        console.log(url)
    let data = form.serialize();
    $.ajax({
        data: data,
