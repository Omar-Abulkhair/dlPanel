...
@section('content')
    ...
    <section class="content-body">
        <div class="col-12 text-right my-2">
            <a href="{{route('dashboard.specialties.create')}}" class="btn btn-relief-primary text-right">Add New</a>
        </div>
        <table id="categories-table" class="display table table-striped responsive nowrap table-bordered" style="width:100%">
            ...
            @foreach($specialties as $specialty)
                <tr>
                    ...
                    <td width="20%">
                        <a href="{{route('dashboard.specialties.edit',$specialty)}}" class="btn btn-primary">Edit</a>
                        <form action="{{route('dashboard.specialties.destroy',$specialty)}}" class="d-inline-block" method="post">
                            {{ method_field('delete') }}
                            @csrf
        ...
        <div class="col-8 col-xs-offset-2 text-xs-center m-auto">
            {{$specialties->links('Panel::layouts.dashboard.pagination')}}
        </div>
